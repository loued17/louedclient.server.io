...
if(count($errors)==0){
	...
	$sql = "INSERT INTO  bookapp (AppoID, Date, Time, individualID,offID) VALUES ('$AppoID','$Date','$Time','$userprofile','$offID') ";
...
if (isset($_POST['cancel'])) {
 ...
 if (count($errors)==0) {
	...
	$query5="DELETE FROM bookapp WHERE AppoID='$AppoID2' AND individualID=('$userprofile') ";
...
if (empty($addname)) {
	array_push($errors,"Official Name is required");
	# code...
}
...
if(count($errors)==0){
	...
	$sqladd = "INSERT INTO  official (officialID, officialname, email, Address, ContactNumber, password,categorey) VALUES ('$addID','$addname','$addEmail','$addAddress','$addContactNumber','$addPassword','$addcategory') ";
...
if (isset($_POST['Delete'])) {
 ...
 if (count($errors)==0) {
	...
	$querydelete="DELETE FROM official WHERE OfficialID='$deleteID' ";
	if ($mysqli -> query($querydelete)) {

		if ($mysqli->affected_rows==0) {
			 array_push($errors,"Wrong Official ID");
