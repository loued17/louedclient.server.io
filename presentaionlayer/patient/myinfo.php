...
<header>
	<h1>Individuals</h1>
		...
		<ul> 
			...
			<li><a href=" searchdoctor.php">Search BRGY. Officials</a></li>
...
<div class="contentP" style="font-weight: bold;">
	...
	<label>ID: <?php echo "" .$_SESSION['UserID'];?></label>
	 	   ...
	 	   <label> Contact Number : <?php echo $col['ContactNumber']; ?></label>
	 	   	 	   <br>
	 	   <br>
	 	   <label> Blood Type : <?php echo $col['Citizenship']; ?></label>
	 	   	 	   <br>
	 	   <br>
...
<?php  if (isset($_POST['appointmenthistory'])) {
			 ?>
		...
		$IID =$mysqli -> real_escape_string($_POST['UserID']);

		$sqlP2="SELECT  * FROM  description   WHERE descID=('$IID') OR descName=('$IID') " ;
		$resultP2=$mysqli->query($sqlP2);
		if(mysqli_num_rows($resultP2)>1){
			while ($rowP2=$resultP2->fetch_assoc()) {

				echo "<tr><td>".$rowP2["descID"]."</td><td>".$rowP2["descName"]."</td><td>".$rowP2["Appointment"]."</td><td>".$rowP2['Note']."</td></tr>";
			}
