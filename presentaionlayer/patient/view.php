...
<body>
	...
	<table class="table2">
		...
		<?php $sql3="SELECT  * FROM bookapp , official   WHERE individualID=('$userprofile') AND  offID=officialID "  ;
		$result3=$mysqli->query($sql3);
		if(mysqli_num_rows($result3)>=1){
			while ($row3=$result3->fetch_assoc()) {

				echo "<tr><td>".$row3["AppoID"]."</td><td>".$row3["Date"]."</td><td>".$row3["Time"]."</td><td>".$row3["offID"]."</td><td>".$row3['officialname']."</td><td>".$row3['Address']."</td><td>".$row3['ContactNumber']."</td><td>".$row3["categorey"]."</td></tr>";
			}
