...
<body>
	...
	<table class="table4">
		...
		if(mysqli_num_rows($result3)>=1){
			while ($row3=$result3->fetch_assoc()) {
				...
				echo "<tr><td>".$row3["AppoID"]."</td><td>".$row3["offID"]."</td><td>".$row3["individualID"]."</td><td>".$row3["Date"]."</td><td>".$row3['Time']."</td></tr>";
			}
    ...
    if (isset($_POST['sendmail'])){
        if(mail($_POST['email'], $_POST['subject'], $_POST['message'],'FROM:delacruzloued@gmail.com')){
            echo "success";
        }else{
            echo "failed";
        }
